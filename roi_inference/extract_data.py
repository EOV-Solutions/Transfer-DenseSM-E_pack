...
os.makedirs(f'roi_inference/regions_data_results/{region}/csv_output', exist_ok=True)
os.makedirs(f'roi_inference/regions_data_results/{region}/csv_output/s1', exist_ok=True)
os.makedirs(f'roi_inference/regions_data_results/{region}/csv_output/combination', exist_ok=True)
s1_folder = f"{root_path}/{region}/data/s1_images"
...
for s1_file in os.listdir(s1_folder):
    if s1_file.endswith('.tif'):
        ...
        tif_path = f'{root_path}/{region}/data/s1_images/S1_{date}.tif'
        break
...
"""Extract NDVI data"""
samples = []
ndvi_folder = f'{root_path}/{region}/data/ndvi_images'
era5_folder = f'{root_path}/{region}/data/weather_images'

df_ndvi, dates, num_ndvi_images = extract_ndvi(ndvi_folder, coordinates)
# Save to CSV
df_ndvi.to_csv(f"{root_path}/{region}/csv_output/ndvi_values.csv", index=False)
...
df_T.to_csv(f"{root_path}/{region}/csv_output/T_values.csv", index=False)
print("Saved Temperature csv!")

# Save precipitation to CSV
df_P.to_csv(f"{root_path}/{region}/csv_output/P_values.csv", index=False)
...
df_static = extract_terrain_soil_texture(
    soil_tif = f"{root_path}/{region}/data/soilgrid_images/SoilGrid_sand_clay_bdod.tif",
    dem_tif = f"{root_path}/{region}/data/dem_images/DEM_elevation_slope_aspect_10m.tif",
    coordinates = coordinates,
    pobj=pobj,
    grid_size=grid_size, 
    pg = point_geometry)

df_static.to_csv(f"{root_path}/{region}/csv_output/extract_dem_soil_from_tif2.csv", index=False)