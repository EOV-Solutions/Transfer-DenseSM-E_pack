...
"""
import collect_soil_moisture.data_available_dates as get_s1_dates
import collect_soil_moisture.data_100m.select_points as select_points
...
def collect_100m_sm(region, root_path, grid_path, landcover_path, tif_folder, start_date='2021-01-01', end_date='2022-12-31'):
    """
    ...
    # Theshold is the percent that sum of selected classes (crop, tree,...) over total area
    filtered_grid = select_points.filtered_grid(root_path, region, grid_path, landcover_path, threshold = 50)
    ...
    if region in  ['india','thaibinh']:
        points_df = select_points.choose_india_points(root_path, filtered_grid, points_df_path)
    elif region == 'china':
        points_df = select_points.choose_china_points(root_path, filtered_grid, landcover_path, points_df_path)
    else:
    ...
    print(f"*****Extract soil moisture from the points in {region} from {start_date} to {end_date} \
            *****Save site (point) information and soil moisture with dates for each site")
    
    # Path to the csv including selected points
    points_df_path = f"{root_path}/{region}/random_points_in_filtered_grid.csv"
    ...
    extract_sm.extract_data_for_region(region, points_df_path, root_path, tif_folder, s1_dates_output_path, site_info_path, output_folder)
...
os.makedirs(f"{root_path}/{region}/map", exist_ok=True)

# Input files ussually in 'map' folder
landcover_path = f'{root_path}/{region}/map/dw_{region}.tif'
grid_path = f'{root_path}/{region}/map/{region}_grid.gpkg'
tif_folder = f'{root_path}/{region}/{region}_tif'
start_date = '2021-01-01'
end_date = '2022-12-31'
collect_100m_sm(region, root_path, grid_path, landcover_path, tif_folder, start_date, end_date)
