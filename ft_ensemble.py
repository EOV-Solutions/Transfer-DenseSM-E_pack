...
class ResultsAnalysis:
    ...
    def scatter_density_fig(self, fig_dir=''):
        ...
        # histogram the data
        valid_mask = np.isfinite(self.y_true) & np.isfinite(self.y_pred)
...
class FinetuneModel:
    ...
    def cal_beta_v3(self, model):
        ...
        with torch.no_grad():
            ...
            fmv_loss = run_loss.cal_mv_loss(mv.narrow(0, 0, source_size), source_y)
            transfer_loss = run_loss.cal_transfer_loss(target_fe, target_y)
            beta = fmv_loss * self.alpha / transfer_loss
            beta = beta.detach().cpu()
        return beta
    ...
    def ft2(self, model):
        if self.beta == 'auto':
            beta = self.cal_beta_v3(model)
            #print(beta)
        else:
            beta = self.beta
        model.fe.to(device)
        model.mv.to(device)
        optimizer_fe = torch.optim.Adam(model.fe.parameters(), lr=self.lr, weight_decay=1e-5)
        optimizer_mv = torch.optim.Adam(model.mv.parameters(), lr=self.lr, weight_decay=1e-5)
        scheduler_fe = ReduceLROnPlateau(optimizer_fe, mode = 'min', factor = 0.5, patience = 50, min_lr = 1e-6)
        scheduler_mv = ReduceLROnPlateau(optimizer_mv, mode = 'min', factor = 0.5, patience = 50, min_lr = 1e-6)
        average_state = AverageModels()
        hist = TrainHist()
        ...
        for epoch in range(self.epoch_Num):
            ...
            # Use RMSE loss to update the learning rate
            val_loss = res.stat[2]
            scheduler_fe.step(val_loss)
            scheduler_mv.step(val_loss)

            # Curren learning rate 
            current_lr_fe = optimizer_fe.param_groups[0]['lr']
            current_lr_mv = optimizer_mv.param_groups[0]['lr']

            if current_lr_fe < 1e-6 or current_lr_mv < 1e-6:
                print(f"Early stopping at epoch {epoch+1} because learning rate < 1e-6")
                break
