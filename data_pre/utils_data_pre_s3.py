...
def extract_S1(geometry, START_DATE, END_DATE, S1_DIR=''):
    ...
    df_all.to_csv("deletethis/s1.csv")
    return df_all
...
class GeeTsExtractor:
    ...
    def download_data_point(self, geometry, scale):
        ...
        bands_df.to_csv("deletethis/weather.csv")
        return bands_df
...
def extract_terrain_soil_texture_v1(geometry, ring_wgs, pobj, grid_size):
    def re_order(aux):
    ...
    soil_terrain_loc['r'] = pobj.r[0] / (9 / grid_size) / 1624
    soil_terrain_loc['c_sin'] = np.sin(pobj.c[0] / (9 / grid_size) / 3856 * np.pi * 2)
    soil_terrain_loc['c_cos'] = np.cos(pobj.c[0] / (9 / grid_size) / 3856 * np.pi * 2)
    print(soil_terrain_loc)
    soil_terrain_df = pd.DataFrame(soil_terrain_loc, index=[0])
    soil_terrain_df.to_csv("deletethis/soil_terrain.csv")
    return soil_terrain_loc


def prepare_grid_data_v1(geo,START_DATE, END_DATE,START_DATE_NDVI,END_DATE_NDVI,ring_wgs,pobj,grid_size):
    # Sentinel-1
    df_S1=extract_S1(geo,START_DATE, END_DATE,S1_DIR='')
    # soil_terrain_loc
    df_Aux=extract_terrain_soil_texture_v1(geo,ring_wgs,pobj,grid_size)
    # NDVI
...
def extract_NDVI_13Q1(polygon_grid,START_DATE_NDVI, END_DATE_NDVI):
    ...
    df_NDVI.to_csv('deletethis/NDVI.csv')
    return df_NDVI


def samples_4_grid_v1(geo, START_DATE, END_DATE, START_DATE_NDVI, END_DATE_NDVI, ring_wgs,pobj, grid_size):
    df_S1, df_NDVI, df_Aux, df_Weather = prepare_grid_data_v1(geo,START_DATE, END_DATE,START_DATE_NDVI,END_DATE_NDVI,ring_wgs,pobj, grid_size)
    ...
    for i in range(len(df_S1)):
        ...
        if len(df_NDVI[index]) != 46:
            print(len(df_NDVI[index]))
            print(f'from {last_ndvi_date - pd.Timedelta(days=363)} to {a_date}')
            print("NDVI last date: ", last_ndvi_date)
            random_value = np.random.choice(NDVI)
            try:
                insert_index = np.where(NDVI == random_value)[0][0] + 1 # Insert after the first occurence
                for _ in range(46 - len(NDVI)): 
                    NDVI = np.insert(NDVI, insert_index, random_value)
                    insert_index += 1
        ...
        """ Uncomment when calculating the mean temperature and precipitation
        if index.sum() < 368:
        ...
        # Get the temperature of the first 8 days
        ...
        # Get the precipitation of the first 8 days
        P = df_Weather[index]['total_precipitation_sum'].values[::8]
        P = normalizingData(np.asarray(P), 0, 0.3)

        # print(f'NDVI.length = {len(NDVI)}, T.length = {len(T)}, P.length = {len(P)}')
        sample = np.concatenate([S1, Aux, NDVI[:46], T[:46], P[:46]])
