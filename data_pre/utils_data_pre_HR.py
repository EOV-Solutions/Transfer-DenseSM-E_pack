...
def parse_S1_platform_orbit(fname):
    ...
    # print("obs_orbit: ",fname.split('_')[7])
...
class GeeS1TsExtractor:
    ...
    def download_data(self):
        ...
        #                                 bands=self.bands,
        #                                 scale=10)
        ts = col.map(lambda image : image.reduceRegions(collection = fc,
                                                        reducer = ee.Reducer.mean(),
                                                        scale = 10).map(lambda feature : feature.set('date', image.date().format('YYYY-MM-dd')))).flatten()
        ts_info = ts.getInfo()
...
def parse_ts_ls(ts_info, sel_band):
    ts = []
    for i in range(len(ts_info['features'])):
        temp = []
        for band in ['date', 'mean']:
            try:
                temp.append(ts_info['features'][i]['properties'][band])
            except:
                temp.append(np.nan)
...
class GeeTsExtractor:
    def __init__(self, product, bands, start_date, end_date, dir_name='', interp=False,save_file=True, custom_collection = None):
        self.product = product
        self.bands = bands
        self.start_date = start_date
        self.end_date = end_date
        self.collection = custom_collection if custom_collection else ee.ImageCollection(product)
        self.interp=interp
    ...
    def download_data(self, geometry, scale):
        ...
        col = self.filtered_collection()
        ts = col.map(lambda image : image.reduceRegions(
            collection = fc,
            reducer = ee.Reducer.mean(),
            scale = scale
        ).map(lambda feature : feature.set('date', image.date().format('YYYY-MM-dd')))).flatten()# map(lambda feature : feature.set('date', image.get('date'))))
        ts_info = ts.getInfo()
    ...
    def download_data_ls(self, geometry, scale):
        def generate_8day_dates(start, end):
            date_list = []
            for year in range(start.year, end.year + 1):
                year_dates = pd.date_range(start = f'{year}-01-01', end = f'{year}-12-31', freq = '8D')
                date_list.extend(year_dates)
            
            return pd.DatetimeIndex(sorted(set(date_list)))
        sel_band = ['date']
        sel_band.extend(self.bands)
        fc = ee.FeatureCollection([geometry])
        col = (self.collection
           .filterBounds(geometry)  # Ensure only images in AOI
           .select('NDVI')  # Only keep NDVI band
           )  
        num_images = col.size().getInfo()
        print('Number of images:', num_images)

        # More efficient reduceRegions (no need to reproject)
        ts = col.map(lambda image:
            image.reduceRegions(
                collection=fc,
                reducer=ee.Reducer.mean(),
                scale=scale
            ).map(lambda feature: feature.set('date', image.date().format('YYYY-MM-dd')))
        ).flatten()

        ts_info = ts.getInfo()
        df = parse_ts_ls(ts_info, sel_band)
        ...
        target_days = generate_8day_dates(start_date, end_date)
        date_list = pd.date_range(self.start_date, self.end_date, freq='1D')
        ...
        df_final = df_interp[df_interp.index.isin(target_days)]
        return df_final
    ...
    def get_and_save_data(self, geometry, site_name, scale=250):
        file_name = f'{self.dir_name}{site_name}.csv'
        if scale == 30: 
            point_df = self.download_data_ls(geometry, scale = scale)
        else:
...
def prepare_grid_data_v1(geo,START_DATE, END_DATE,START_DATE_NDVI,END_DATE_NDVI,ring_wgs,pobj, save_path):
    # Sentinel-1
    ...
    df_NDVI=extract_NDVI_from_landsat(geo,START_DATE_NDVI, END_DATE_NDVI)
    df_NDVI.to_csv(save_path)
    ...
    # # Get precipitation with higher resolution
    # CHIRPS_PRODUCT = "UCSB-CHG/CHIRPS/DAILY"
    # ex_P = GeeTsExtractor(CHIRPS_PRODUCT, ['precipitation'], START_DATE_NDVI, END_DATE_NDVI)

    # df_P = ex_P.download_data_point(point_geo, 5566)

    # # Concate the data of CHIRPS to ERA-5

    # df_Weather = pd.concat([df_Weather, df_P], axis = 1)
    # df_Weather['precipitation'] = df_Weather['precipitation']/1000.0

    # return df_S1,df_NDVI,df_Aux,df_Weather
    return None, df_NDVI, None, None
...
def extract_NDVI_13Q1(polygon_grid,START_DATE_NDVI, END_DATE_NDVI):
    ...
    df_MOD = ex.get_and_save_data(polygon_grid, 'temp', 250)

    MOD_PRODUCT = "MODIS/061/MYD13Q1"
    ex = GeeTsExtractor(MOD_PRODUCT, bands, START_DATE_NDVI, END_DATE_NDVI)
    df_MYD = ex.get_and_save_data(polygon_grid, 'temp', 250)
    df_NDVI = pd.concat([df_MOD, df_MYD])
    df_NDVI = df_NDVI.sort_index()
    return df_NDVI

def mask_clouds(image):
    # Get cloud mask from QA_pixel band 
...
def extract_NDVI_from_landsat(polygon_grid, START_DATE_NDVI, END_DATE_NDVI):
    L8_PRODUCT = "LANDSAT/LC08/C02/T1_L2"
    L9_PRODUCT = "LANDSAT/LC09/C02/T1_L2"
    bands = ['NDVI']

    # Create ImageCollection and compute NDVI
    landsat = ee.ImageCollection(L8_PRODUCT).filterDate(START_DATE_NDVI, END_DATE_NDVI).merge(
                        ee.ImageCollection(L9_PRODUCT)).filterDate(START_DATE_NDVI, END_DATE_NDVI)
    landsat = landsat.map(mask_clouds).map(compute_NDVI)

    ex_ls = GeeTsExtractor(product=L8_PRODUCT, bands=bands, interp=True ,start_date=START_DATE_NDVI, end_date=END_DATE_NDVI, custom_collection=landsat)
    df_ls = ex_ls.get_and_save_data(polygon_grid, 'temp', scale = 30)
...
def samples_4_grid_v1(geo, START_DATE, END_DATE, START_DATE_NDVI, END_DATE_NDVI, ring_wgs,pobj):
    ...
    for i in range(len(df_S1)):
        ...
        last_ndvi_date = df_NDVI[df_NDVI.index <= a_date].index[-1]
        index = (df_NDVI.index <= a_date) & (df_NDVI.index > (last_ndvi_date - pd.Timedelta(days=363)))
        NDVI_dates = df_NDVI[index].index 
        NDVI = normalizingData(np.asarray(df_NDVI[index]['NDVI']), -2000, 10000)
        # NDVI = normalizingData(np.asarray(df_NDVI[index]['NDVI'].iloc[::8]), -1, 1)
        
        if len(NDVI) != 46:
            print(len(NDVI))
            print(f'from {last_ndvi_date - pd.Timedelta(days=363)} to {a_date}')
            print("NDVI last date: ", last_ndvi_date)
        ...
        # if index.sum() < 368:
        #     print("ndiv's first date : ", df_NDVI.index[0])
        #     dif = 368 - index.sum() - 1
        #     # Adjust the date range to have enough 368 items
        #     start_date = df_NDVI.index[0] - pd.Timedelta(days = dif)
        #     end_date = df_NDVI.index[0] + pd.Timedelta(days = index.sum())
        #     # end_date = start_date + pd.Timedelta(days = 368)
        #     index = (df_Weather.index >= start_date) & (df_Weather.index <= end_date)
        #     expected_dates = pd.date_range(start=start_date, end=end_date, freq="D")

        #     # Find which dates are missing in df_Weather
        #     missing_dates = expected_dates.difference(df_Weather.index)
                        
        # T = df_Weather[index]['temperature_2m'].values.reshape(-1, 8).mean(axis=1)

        df_weahter_filtered = df_Weather[df_Weather.index.isin(NDVI_dates)]
        T = df_weahter_filtered['temperature_2m'].values
        # T = df_Weather[index]['temperature_2m'].values[::8]
        # Adjust the minimum and maximum for each region
        T = normalizingData(np.asarray(T), 273.15, 318.15)

        # Calculate the mean of the total precipitation for each 8 days
        # P = df_Weather[index]['total_precipitation_sum'].values.reshape(-1, 8).mean(axis=1)
        # Get the total precipitation for each 8 days
        # P = df_Weather[index]['total_precipitation_sum'].values[::8]
        P = df_weahter_filtered['total_precipitation_sum'].values
        P = normalizingData(np.asarray(P), 0, 0.3)

        sample = np.concatenate([S1, Aux, NDVI, T, P])
