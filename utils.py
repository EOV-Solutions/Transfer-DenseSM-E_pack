...
def select_station_samples(input_all, network_name, nstation=1, seed=1):
    ...
    yy = input_train.groupby('station').count()['angle'].sample(n=nstation, random_state=seed)
    selected_station = yy.index.tolist()
    missing_stations = set(input_all['station'].unique()) - set(selected_station)
    print("Stations not present in selected_station:", missing_stations)
...
def prepare_train_val_data_highres(samples_part, train_val_index, batch_size=128,br=1):
    xtrain, ytrain = split_train_val(samples_part, train_val_index[0])
    xtest, ytest = split_train_val(samples_part, train_val_index[1])
    ...
    val_data = test_set_GPU(xtest, ytest)
    train_data = test_set_GPU(xtrain, ytrain)
    return trainloader, val_data, targetloader, train_data
...
def split_train_val(samples_part, train_index=[]):
    if len(train_index)==0:
        ...
        ytrain = np.asarray(samples_part.loc[:, 'sm']).astype(np.float32)
    else:
        xtrain = np.asarray(samples_part.loc[train_index, '0':'152']).astype(np.float32)
        xtrain = input_reorganize(xtrain)
        ytrain = np.asarray(samples_part.loc[train_index, 'sm']).astype(np.float32)
